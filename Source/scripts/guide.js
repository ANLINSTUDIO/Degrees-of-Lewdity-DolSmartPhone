PhoneMod.Guide = {}


PhoneMod.Guide.photo = [
    {
        image: "img/guide/photo/1.png",
        text: "摄像APP只在特殊的时刻有用。平时打开时，它不会显示任何东西。",
        alt: ""
    },
    {
        image: "img/guide/photo/2.png",
        text: "当你前往相册时，可以看到那些值得拍照的事情（其实就是任务）。",
        alt: ""
    },
    {
        image: "img/guide/photo/3.png",
        text: "当你完成任务时，你的手机会开始跳动（不论当前手机是否可用）。此时打开你的手机，就会自动弹出拍照界面。",
        alt: ""
    },
    {
        image: "img/guide/photo/4.png",
        text: "在拍照界面，你的选择有点击拍照按钮，此时会将照片存入相册；或者点击锁屏：这不会拍照。<br>完成后手机会自动收起。",
        alt: ""
    },
    {
        image: "img/guide/photo/5.png",
        text: `之后打开相册，会发现刚才的任务已经完成，已经拍摄到了刚才的照片。<br>
        照片包含很多属性：诱惑和质量决定照片的吸引度，你拍照时的人物性奋和诱惑决定照片诱惑，你的摄影技术很大程度上决定照片的质量。<br>
        同时，一张照片可能会影响你的相应的名声。<br>
        你可以将照片发布到<span class="yellow">小黄书</span>，来获得属性、名声的增益和金钱的盈利。`,
        alt: ""
    },
    {
        image: "img/guide/photo/6.png",
        text: `一张照片可以重复使用，但是其热度会大幅减少。<br>
        如果你认为一张照片的质量不够高，你可以将照片删除，再重新完成一遍任务，那么就可以获得更高质量的照片了。<br>
        每次拍摄都会增加你的摄影技术，摄影技术越高，拍出来的照片质量越高。`,
        alt: ""
    }
];

PhoneMod.Guide.yenote = [
    {
        image: "img/guide/yenote/1.png",
        text: "欢迎来到小黄书。小黄书是一个社交平台，人们可以在这里发送各种文章和照片，拓宽自己的社交。小黄书支持发送付费内容，致力为独立媒体工作者提高一个表现自己的平台。",
        alt: ""
    },
    {
        image: "img/guide/yenote/2.png",
        text: "APP的页面如图。你可以点击你的头像来修改你的名字，点击右上方加号跳转到相册来发布文章。你可以删除自己发布的文章。",
        alt: ""
    },
    {
        image: "img/guide/yenote/3.png",
        text: "吸引度由任务风险、照片诱惑和照片质量共同决定；吸引度、时间、相应名声和付费价格共同影响着文章的热度；文章可以设定为付费内容，最大金额由吸引度决定，且价格对热度呈指数影响。",
        alt: ""
    },
    {
        image: "img/guide/yenote/4.png",
        text: "评论区的评论会对你的人物属性造成正面或负面的影响。浏览、点赞和评论都会增加你相应的名声，而名声越高热度也就会越高，但是热度还是禁不起时间的沉淀。",
        alt: ""
    },
    {
        text: "当你的文章为免费内容时，可能收到来自点赞你的文章的人的随机打赏。最高金额为照片的吸引度。当你设置为付费内容时，每一位浏览都会给你带来设定的金额。",
    }
];

PhoneMod.Guide.memo = [
    {
        image: "img/guide/memo/1.png",
        text: `这里是备忘录，在这你可以设定自己单个的待办事项。<br>
        任务可以通过单击以勾选完成，也可以通过点击右侧的圆圈将其标记为重要任务。<br>
        当任务处于完成状态，右侧的圆点会变成红色，此时点击的效果为删除。<br>
        你可以选择性地开启自动排序，这会将重要任务置顶，完成任务置底。`,
        alt: ""
    }
];

PhoneMod.Guide.contacts = [
    {
        image: "img/guide/contacts/1.png",
        text: `这里是通讯录。玩家可以在非活动期间拨打NPC的电话，与他们实现和面对面交流同样效果的聊天，甚至可以触发特殊角色的特殊对话（例如为玩家添加了找兰德里使用钱消除犯罪记录的玩选项）。`,
        alt: ""
    }, 
    {
        image: "img/guide/contacts/2.png",
        text: `模组中的通讯录默认为空，玩家与大部分<span class="teal">设定上持有着智能手机</sapn>的角色好感度达到一定程度后，便可以亲自询问其电话号码以添加通讯录。`,
        alt: ""
    }, 
    {
        image: "img/guide/contacts/3.png",
        text: `与兰德里通话，你可以<span class="gold">使用金钱消除犯罪记录</span>而不要再寻找黑匣子（如果你当时已经接了找黑匣子的任务除外）。`,
        alt: ""
    }, 
    {
        image: "img/guide/contacts/4.png",
        text: `与裁缝通话，你可以向他<span class="gold">订购服装修补服务</span>，板条箱会直接送来，你可以直接在衣柜里提交你要修补的衣服。`,
        alt: ""
    }, 
];

PhoneMod.Guide.secondphoneshop = [
    {
        image: "img/guide/secondphoneshop/1.png",
        text: `这里是地下手机店。<br>
        玩家可以在这里 <span class="teal">购买手机 、出售二手手机 并且破解盗窃而来的手机</span>。
        在这里你可以无忧无虑地售出被盗的手机。在这里售出的手机价格会<span class="gold">低于官方手机店10%</span>，但是会<span class="pink">增加老冯的好感</span>。<br>
        在这里你可以破解盗窃而来的手机，价格为其<span class="gold">售出价格的20%</span>。`,
        alt: ""
    }, 
]

PhoneMod.Guide.stealphone = [
    {
        image: "img/guide/stealphone/1.png",
        text: ``,
        alt: ""
    }, 
]


// 启动教程
PhoneMod.Guide.startTutorial = function(data) {
    let tutorialCompleted = false;
    if (V.Phone.Guide) {
        for (let key in PhoneMod.Guide) {
            if (PhoneMod.Guide[key] === data) {
                tutorialCompleted = V.Phone.Guide.hasOwnProperty(key)
                break
            }
        }
    }
    if (!tutorialCompleted) {
        PhoneMod.Guide.currentStep = 1;
        PhoneMod.Guide.currentData = data;
        PhoneMod.Guide.totalSteps = data.length;
        PhoneMod.Guide.touchStartX = 0;
        PhoneMod.Guide.touchEndX = 0;

        const smartphone_guide = document.getElementById("smartphone_guide")
        if (smartphone_guide) {
            smartphone_guide.remove();
        }

        new Wikifier(document.querySelector("body"), "<<smartphone_guide>>")
        // 点击步骤指示器切换

        const stepsContainer = document.getElementById('tutorialSteps');
        data.forEach((_, index) => {
            const stepNumber = index + 1;
            const indicator = document.createElement('span');
            indicator.className = `step-indicator ${stepNumber === currentStep ? 'active' : ''}`;
            indicator.dataset.step = stepNumber;
            indicator.textContent = stepNumber;
            
            // 添加点击事件
            indicator.addEventListener('click', () => {
                PhoneMod.Guide.goToStep(stepNumber);
            });
            
            stepsContainer.appendChild(indicator);
        });
        document.getElementById('totalSteps').textContent = PhoneMod.Guide.totalSteps;

        const overlay = document.getElementById('tutorial-overlay');
        overlay.classList.remove('hidden');
        PhoneMod.Guide.updateTutorialStep(PhoneMod.Guide.currentStep);
        
        // 添加触摸事件监听
        const content = document.querySelector('.tutorial-content');
        content.addEventListener('touchstart', PhoneMod.Guide.handleTouchStart, false);
        content.addEventListener('touchend', PhoneMod.Guide.handleTouchEnd, false);
    }
}

PhoneMod.Guide.resetGuide = function() {
    V.Phone.Guide = []
    PhoneMod.msgSend("教程进度已经清空，现在你将可以重新再次阅读所有的教程。")
}

// 关闭教程
PhoneMod.Guide.closeTutorial = function() {
    const overlay = document.getElementById('tutorial-overlay');
    overlay.classList.add('hidden');
    
    // 移除触摸事件监听
    const content = document.querySelector('.tutorial-content');
    content.removeEventListener('touchstart', PhoneMod.Guide.handleTouchStart);
    content.removeEventListener('touchend', PhoneMod.Guide.handleTouchEnd);

    const smartphone_guide = document.getElementById("smartphone_guide")
    if (smartphone_guide) {
        smartphone_guide.remove();
    }
}

// 更新教程步骤
PhoneMod.Guide.updateTutorialStep = function(step) {
    const data = PhoneMod.Guide.currentData[step - 1];
    if (!data) return;
    
    // 更新图片
    const image = document.getElementById('tutorialImage');
    if (data.image){
        image.style.display = ''
        image.src = data.image;
        image.alt = data.alt;
    } else {
        image.style.display = 'none'
    }
    
    // 更新文字（添加淡入动画）
    const textElement = document.getElementById('tutorialText');
    textElement.style.opacity = '0';
    setTimeout(() => {
        textElement.innerHTML = data.text;
        textElement.style.opacity = '1';
    }, 200);
    
    // 更新步骤指示器
    document.querySelectorAll('.step-indicator').forEach((indicator, index) => {
        if (index + 1 === step) {
            indicator.classList.add('active');
        } else {
            indicator.classList.remove('active');
        }
    });
    
    // 更新当前步骤显示
    document.getElementById('currentStep').textContent = step;
    
    // 更新按钮状态
    const prevBtn = document.getElementById('prevBtn');
    const nextBtn = document.getElementById('nextBtn');
    
    prevBtn.disabled = step === 1;
    
    if (step === PhoneMod.Guide.totalSteps) {
        nextBtn.innerText = "明白了"
    } else {
        nextBtn.innerText = "下一步"
    }
}

// 下一步
PhoneMod.Guide.nextStep = function() {
    if (PhoneMod.Guide.currentStep < PhoneMod.Guide.totalSteps) {
        PhoneMod.Guide.currentStep++;
        PhoneMod.Guide.updateTutorialStep(PhoneMod.Guide.currentStep);
    } else {
        PhoneMod.Guide.completeTutorial();
    }
}

// 上一步
PhoneMod.Guide.prevStep = function() {
    if (PhoneMod.Guide.currentStep > 1) {
        PhoneMod.Guide.currentStep--;
        PhoneMod.Guide.updateTutorialStep(PhoneMod.Guide.currentStep);
    }
}

// 跳转到指定步骤（通过点击步骤指示器）
PhoneMod.Guide.goToStep = function(step) {
    if (step >= 1 && step <= PhoneMod.Guide.totalSteps) {
        PhoneMod.Guide.currentStep = step;
        PhoneMod.Guide.updateTutorialStep(PhoneMod.Guide.currentStep);
    }
}

// 完成教程
PhoneMod.Guide.completeTutorial = function() {
    PhoneMod.Guide.closeTutorial();
    for (let key in PhoneMod.Guide) {
        if (PhoneMod.Guide[key] === PhoneMod.Guide.currentData) {
            V.Phone.Guide[key] = true
            break
        }
    }
}

// 触摸事件处理
PhoneMod.Guide.handleTouchStart = function(event) {
    PhoneMod.Guide.touchStartX = event.touches[0].clientX;
}

PhoneMod.Guide.handleTouchEnd = function(event) {
    PhoneMod.Guide.touchEndX = event.changedTouches[0].clientX;
    PhoneMod.Guide.handleSwipe();
}

// 处理滑动
PhoneMod.Guide.handleSwipe = function() {
    const swipeThreshold = 50; // 滑动阈值
    const diff = PhoneMod.Guide.touchStartX - PhoneMod.Guide.touchEndX;
    
    if (Math.abs(diff) > swipeThreshold) {
        if (diff > 0) {
            // 向左滑动，下一步
            PhoneMod.Guide.nextStep();
        } else {
            // 向右滑动，上一步
            PhoneMod.Guide.prevStep();
        }
    }
}
